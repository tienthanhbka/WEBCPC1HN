...
<script>
import { UpdateMissionVacation } from "@/api/missionVacation";
  ...
  methods: {
    ...
    initForm() {
      if (this.row) {
        ...
        this.form.Email = this.row.Email.split(";");
        this.form.Number = this.row.Number;
        this.listDate = this.row.ExtraData
          ? JSON.parse(this.row.ExtraData)
          : "";
    ...
    selectTimeEnd(val) {
      ...
      const TimeS = new Date(this.form.TimeStart).getTime();
    ...
    editMission() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          UpdateMissionVacation({ MissionVacationInfo: this.form }).then(
            response => {
              if (response.RespCode == 0) {
                this.$notify({
                  title: "Thành công",
                  message: "Chỉnh sửa phiếu thành công",
                  type: "success",
                  position: "top-left"
                });
