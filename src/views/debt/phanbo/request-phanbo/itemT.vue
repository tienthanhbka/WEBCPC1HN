<template>
    <div>
        ...
        <el-button v-if="phanboCoupon.status==1||phanboCoupon.status ==4" style="position:absolute;left:0; color: #16a085;
    padding: 16px 18px;" @click="openModal" ><i class="fas fa-plus"></i></el-button>
        </el-tooltip>
        <el-table :key="tKey"
      ...
      >
        <el-table-column type="expand">
          <template slot-scope="scope">
            <type-customer-t :item=scope.row :phanboCoupon="phanboCoupon" ></type-customer-t>
          </template>
        </el-table-column>
        <el-table-column align="center" width="75px" label="STT">
          <template slot-scope="scope"><i class="fas fa-pills" style="color:#16a085"></i> {{ scope.$index + 1 }}
              </template>
        </el-table-column>
        ...
        <el-table-column width="170" label="DVT">
          <template slot-scope="{row}">{{row['Base Unit of Measure']}}</template>
        </el-table-column>
