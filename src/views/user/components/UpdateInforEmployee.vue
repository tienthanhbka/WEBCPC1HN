<template>
  <div>
    ...
    >
      ...
      <el-form-item label="Địa bàn" prop="Place">
        <el-select
          v-model="form.Place"
          multiple
          filterable
...
export default {
  ...
  methods: {
    ...
    querySearch8(queryString, cb) {
      ...
      var results = queryString
        ? links.filter(this.createFilter8(queryString))
        : links;
      cb(results);
    },
    handleIconClick8(ev) {
    ...
    check() {
      console.log(this.form.TranningTime);
      if (
        this.form.EmployeeName == null ||
        this.form.Img == null ||
        this.form.Office == null ||
      ...
      if (this.check()) {
        ...
        UpdateInfor(this.form).then(res => {
          this.$notify({
            title: "Thành công",
            message:
              "Cập nhật thành công. Vui lòng cập nhật thông tin gia đình",
            type: "success",
            position: "top-left"
          });
          alert(
            "Bạn đã nhập thông tin cá nhân xong. Vui lòng cập nhập thông tin gia đình ở nút cập nhật trong phần Thông tin gia đình ở phí bên dưới!"
          );
          this.$emit("click-something", "ok");
        });
      } else {
