<template>
  <div class="group-block" style="padding:20px">
    ...
    >
      <el-table-column width="50px" label="STT" align="center">
          ...
          <div class="dat-cell" label="Action">
            <el-dropdown size="small" split-button type="primary">
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                >
                <el-dropdown-item v-if="Admin == 1 || Admin == 10">
                  <router-link :to="'/STK/' + scope.row.EmployeeID">
                    <el-button
                      style="width: 100%;"
                      @click="accountNumber(scope.row)"
                      size="mini"
                      ><i class="fas fa-credit-card"></i>STK</el-button
                    ></router-link
                  >
                </el-dropdown-item>
      ...
      <el-table-column label="Họ và tên" min-width="140">
        <template slot-scope="scope">
      ...
      <el-table-column label="Cập nhật gần nhất" min-width="150">
        <template slot-scope="scope">
    ...
    ></el-pagination>
    <el-dialog :visible.sync="dialogFormAccept" custom-class="big-modal">
    ...
    <el-dialog :visible.sync="dialogFormNoAccept" custom-class="default-modal">
      <h3 class="title-dialog" slot="title">
    ...
    <!-- <el-dialog
      ...
      :visible.sync="dialogFormAccept"
      custom-class="bid-modal"
    >
      <acceptInfor
        :user="user"
        @click-something="acceptOK"
    ...
    <el-dialog :visible.sync="dialogFormViewExcel" custom-class="big-modal">
      <h3 class="title-dialog" slot="title">
    ...
    <el-dialog :visible.sync="dialogFormRecordHR" custom-class="default-modal">
      <h3 class="title-dialog" slot="title">
        <i class="fas fa-id-card"></i>
        <span>{{ titleDialog }}</span>
      </h3>
    ...
    <el-dialog :visible.sync="dialogFormOTC" custom-class="default-modal">
      <h3 class="title-dialog" slot="title">
        <i class="fas fa-users"></i>
        <span>{{ titleDialog }}</span>
      </h3>
    ...
    <el-dialog :visible.sync="dialogFormKH" custom-class="default-modal">
      <h3 class="title-dialog" slot="title">
        <i class="fas fa-users"></i>
        <span>{{ titleDialog }}</span>
      </h3>
...
export default {
  ...
  methods: {
    recordHR(row) {
      ...
      this.titleDialog = "Hồ sơ nhân sự";
    },
    recordOK() {
    ...
    cancelInfo() {
      ...
      this.titleDialog = "Yêu cầu cập nhật thông tin";
    },
    viewExcel() {
      this.dialogFormViewExcel = true;
      this.titleDialog = "Xem báo cáo";
    ...
    Export1() {
      ...
      GetListEmployee2({ search: this.search, pageSize: 5000, page: 1 }).then(
        response => {
    ...
    Export11() {
      ...
      GetListEmployee({ search: this.search, pageSize: 5000, page: 1 }).then(
        response => {
          var data = response.Data;
          import("@/vendor/Export2Excel").then(excel => {
            excel.export_json_to_excel({
    ...
    Export13() {
      ...
      const tHeader = [
        ...
        "Ngày vào công ty",
        "Ngày nghỉ việc",
        "Đơn xin việc theo mẫu (viết tay)",
        "Kê khai lý lịch theo mẫu (có dán ảnh + xác nhận)",
        "Bản tóm tắt quá trình làm việc, công tác (CV)",
        "Giấy khám sức khỏe (không quá 6 tháng)",
        "Bản sao bằng cấp (công chứng)",
        "Bản sao hộ khẩu (công chứng)",
        "Bản sao giấy khai sinh (công chứng)",
        "Bản sao CMND (công chứng)",
        "Hình (3x4) (ghi tên và ngày sinh ở mặt sau)",
        "Bản cam kết công ty",
        "Quyết định nghỉ việc (bắt buộc nếu đã đi làm)",
        "Cam kết 02",
        "Biên bản thỏa thuận (đối với NVKD)",
        "Ghi chú"
      ];
      const filterHeader = [
        "EmployeeCode",
      ...
      GetRecordHR({
        ...
        var data = response.RecordHRLst.map(item => {
          let obj = {
            item1: "",
            item2: "",
            item3: "",
          ...
          if (item.Name) {
            if (item.Name.includes("Đơn xin việc theo mẫu (viết tay)")) {
              obj.item1 = "✓";
            }
            if (
              item.Name.includes(
            ...
            if (item.Name.includes("Giấy khám sức khỏe (không quá 6 tháng)")) {
              obj.item4 = "✓";
            }
            if (item.Name.includes("Bản sao bằng cấp (công chứng)")) {
              obj.item5 = "✓";
            }
            if (item.Name.includes("Bản sao hộ khẩu (công chứng)")) {
              obj.item6 = "✓";
            }
            if (item.Name.includes("Bản sao giấy khai sinh (công chứng)")) {
              obj.item7 = "✓";
            }
            if (item.Name.includes("Bản sao CMND (công chứng)")) {
              obj.item8 = "✓";
            }
            if (
              item.Name.includes("Hình (3x4) (ghi tên và ngày sinh ở mặt sau)")
            ) {
              obj.item9 = "✓";
            }
            if (item.Name.includes("Bản cam kết công ty")) {
              obj.item10 = "✓";
            }
            if (
              item.Name.includes(
            ...
            if (item.Name.includes("Cam kết 02")) {
              obj.item12 = "✓";
            }
            if (item.Name.includes("Biên bản thỏa thuận (đối với NVKD)")) {
              obj.item13 = "✓";
            }
          }
    ...
    viewDetail(row) {
      ...
      this.titleDialog = "Quản lý nhóm";
    },
    setKH(row) {
      this.userofIT = row;
      this.dialogFormKH = true;
      this.titleDialog = "Lấy khách hàng";
    },
    setKHOK() {
    ...
    setOTC(row) {
      ...
      this.titleDialog = "Loại nhân viên";
    },
    setOTCOK() {
    ...
    acceptInfor(row) {
      ...
      this.titleDialog = "Phê duyệt thông tin";
