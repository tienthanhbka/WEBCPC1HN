<template>
  <div>
    ...
    >
      ...
      <el-table-column label="Upload file KH" min-width="150">
       ...
       <el-table-column label="File" min-width="150">
        <template slot-scope="{row}">
          <div >
            <!-- <li v-for="item in scope.LinkFile">
              {{ item }}
            </li> -->
          </div>

           <span style="margin-left: 10px;" v-for="item in row.LinkFile" :key="item.file">
             <el-tooltip content="dowload file"> <el-button size="mini" type="warning" ><a :href="'https://icpc1hn.work/api/'+'File/GetImageCustomer/'+item.file+'?token='+token+'&CodeCustomer='+row.No_"><i class="fas fa-file-download"></i></a></el-button></el-tooltip>
            <!-- {{ item.file }} -->
          </span>
        </template>
      </el-table-column>
    ...
    ></el-pagination>
     ...
     <el-dialog title="Upload file khách hàng" :visible.sync="dialogFormUpload">
        <uploadFile :row="row" @click-something="uploadOK"></uploadFile>
...
import { GetCustomerBFO,GetInforFile } from "@/api/customerBFO";
      ...
      GetInforFile(req).then(res => {
          ...
          res.Data.forEach(function(item,index,arr){
            if(item.LinkFile != null){
              //item.LinkFile = item.LinkFile.split(",");
             
              var a = item.LinkFile.split(",");
               item.LinkFile = [];
              a.forEach(function(item2,index2,arr2){
                item.LinkFile.push({file: item2})
              })
          ...
          console.log(this.tableData[0].LinkFile)
