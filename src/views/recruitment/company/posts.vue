<template>
  <div class="group-block" style="padding:20px">
    ...
    >
      ...
      <el-table-column width="200px" label="Phân loại">
        <template slot-scope="scope">
...
import { GetPostByUser, DelPost } from "@/api/work-dtp/post";
...
export default {
  ...
  methods: {
    ...
    viewPost(row) {
      ...
      this.titleDialog = "Chi tiết bài đăng";
      this.modeView = 2;
      this.rowData = row;
    },
    addOK() {
    ...
    approve(row) {
      this.$confirm(
        row.Status === 3
          ? "Đóng bài đăng: " + row.Title + ". Continue?"
          : "Public bài đăng: " + row.Title + ". Continue?",
        "Xác nhận",
        {
    ...
    delPost(row) {
      this.$confirm("Xóa bài đăng. Continue?", "Xác nhận", {
        confirmButtonText: "Xác nhận",
        cancelButtonText: "Hủy",
        type: "error"
      })
