<template>
  <div class="group-block" style="padding:20px">
    ...
    >
      ...
      <el-table-column width="200px" prop="WorkPosition" label="Vị trí">
        <template slot-scope="scope">
      ...
      <el-table-column width="150px" label="Số lượng tuyển">
        <template slot-scope="scope">
          <div class="dat-cell" label="Số lượng tuyển">
            {{ scope.row.QuantityCandidate }}
          </div>
        </template>
      </el-table-column>
      <el-table-column width="140px" label="Mức lương">
        <template slot-scope="scope">
          <div class="dat-cell" label="Mức lương">
    ...
    >
      ...
      <create :company="group" :row="rowData" @updateOK="updateOK"></create>
    </el-dialog>
    <el-dialog
...
import Create from "./component/createRecruitment";
import Candidate from "../component/lstCandidate";
import {
  GetRecruitmentByUser,
  DelRecruitment
  ...
  methods: {
    ...
    fetchGroup() {
      ...
      GetCompanyByUser(req).then(res => {
        this.groupLst = res.CompanyLst;
        this.group = this.groupLst[0].CompanyID;
      });
    },
    fetchData() {
      this.loadingTableData = true;
      this.tableData = [];
      GetRecruitmentByUser({ CompanyID: this.group }).then(res => {
        if (res.RespCode === 0) {
