          ...
          <el-card class="box-card-list-item">
            <div slot="header" class="clearfix">
              ...
              <span
                ...
                ><span
                  style="color:#000000;font-size: 13.333px;font-weight: 700;"
                  >Mô tả: </span
...
export default {
  ...
  methods: {
    ...
    addRate() {
      ...
      _this.lines1.forEach(function(item1, index1, arr1) {
        // console.log(item1);
        // console.log(
        //   Number(item1.Quantity) >= Number(item1.Quota.replace(/[^0-9]/g, ""))
        // );
        if (
          Number(item1.Quantity) >= Number(item1.Quota.replace(/[^0-9]/g, ""))
        ) {
  ...
  created() {
    //console.log(this.employee);
    this.$prompt(
      "Nhập loại nhân viên để đánh giá (TV, CT, BN)",
      "Loại nhân viên",
      {
      ...
      .then(({ value }) => {
        ...
        if (this.$route.params.username && this.$route.params.token) {
          // console.log(this.$route.params.username, this.$route.params.token);
