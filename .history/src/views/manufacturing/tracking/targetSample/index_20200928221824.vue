        ...
        <el-table-column label="Trạng thái" width="200">
          <template slot-scope="scope">
            <div class="dat-cell" label="Trạng thái">
              <el-tag :type="scope.row.Status | toStatusColor">
                {{ scope.row.Creator | toStatusText }}</el-tag
              >
            </div>
          </template>
        </el-table-column>
...
export default {
  filters: {
    toStatusText(val) {
      if (val == 1) {
        return "Chưa tạo chỉ tiêu";
      } else if (val == 2) {
        return "Đã tạo chỉ tiêu";
      }
    },
    toStatusText(val) {
      if (val == 1) {
