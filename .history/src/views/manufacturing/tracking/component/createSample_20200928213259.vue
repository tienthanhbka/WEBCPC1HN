<template>
  <div>
    ...
    >
      ...
      <el-row :gutter="20">
        <el-col :xs="24" :sm="12" :lg="12">
          <el-form-item label="Điều kiện lưu" prop="TypeSample">
            <el-select
              class="selectIDGroup"
...
export default {
  ...
  methods: {
    ...
    addSample(re) {
      this.$refs[re].validate(valid => {
        if (valid) {
          ...
          db.ref("ProductList").on("value", snap => {
            if (snap.val()) {
              Object.values(snap.val()).forEach(element => {
                console.log(element);
                console.log(this.Sample.ProductName);
                console.log(element.ProductName != this.Sample.ProductName);
    ...
    changeTypeSample(val) {
      let a = Math.floor(Math.random() * 10001);
      if (val == "Lưu thường") {
        this.Sample.SampleName = "SXT" + a;
      } else if (val == "Lão hóa") {
        this.Sample.SampleName = "SXL" + a;
      } else if (val == "Tủ lạnh") {
        this.Sample.SampleName = "SXTL" + a;
      } else {
        this.Sample.SampleName = "SXBB" + a;
      }
    },
    initForm() {
      if (this.sample) {
        ...
        this.Sample.StartingDate = this.sample.StartingDate;
      } else {
        this.Sample = {
          SampleID: "",
          SampleName: "",
