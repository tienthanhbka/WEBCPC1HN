<template>
  <div class="my-mission-container" id='couponTabHeaderBtn'>
     <div class="inline-block">
       ...
       <div class="inline-block">
        <el-input style="width: 70%; " class="search"   v-on:keyup.enter="Search" placeholder="Tìm kiếm"  v-model="search"></el-input>
      ...
      <!-- <div class="inline-block">
       ...
       <el-button type="success" @click="ExportExcel">Xuất Excel</el-button>
        ...
        <el-button type="warning" @click="ExportExcelNOTRole">Xuất Excel Chưa Đăng Ký</el-button>
        </div>
      ...
      >
        <el-table-column align="center" width="120px"  label="STT">
          <template slot-scope="scope">{{ (scope.$index + 1) + (currentPage - 1)*pageSize }} 
             <el-tooltip content="Duyệt" placement="top"><el-button @click="accept(scope.row)" type="text" >  <i class="fas fa-check-double"></i></el-button></el-tooltip>
         ...
         <el-table-column width="120px"  label="Trạng thái" 
          :filters="[{text: 'Đạt', value: '10'},{text: 'Đã chấm điểm', value: '6'},{text: 'Không đạt', value: '5'}, {text: 'Đã nộp video', value: '4'}, {text: 'Đã thực hiện role', value: '3'}, {text: 'Đã xếp lịch', value: '2'}, {text: 'Mới tạo', value: '1'},{text: 'Hủy', value: '0'}]">
        ...
        <el-table-column prop="NameAccept"  label="Giám sát">
         ...
         <el-table-column  prop="TimeCreate" label="Tg đăng ký"></el-table-column>
...
import { GetRoleGroup,GetTeacherLst,GetAcceptLst,GetEmployeeNOTRole,OpenVideo,CloseVideo } from "@/api/roleplay";
...
export default {
  ...
  components: {
    Create,AddTeacher,Accept,AddTime,AddAccept
  },
  data() {
    return {
      tableData: [],
  ...
  methods: {
      ...
      ExportExcel(){
              ...
              const tHeader = ['Trạng thái','TDV','Nhóm','Người kiểm tra','Giám sát','Điểm','Điểm người giám sát','Ngày','Thời gian','Sản phẩm', 'Ghi chú' ]
              const filterHeader = ['Status','NameEmployee','IDGroup','NameTeacher','NameAccept','Point','PointAccept','Date','TimeStart','Product', 'Note']
              GetRoleGroup({
                token: Cookies.get('token'),
                 month: this.month,
