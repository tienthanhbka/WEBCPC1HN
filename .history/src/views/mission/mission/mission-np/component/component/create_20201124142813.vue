<template>
  <modal id="create-mission-vacation" ref="create-mission-vacation"  v-cloak>
    <template slot="title">Tạo phiếu nghỉ phép/ làm bù</template>
    <div></div>
    <div slot="body">
      <el-form ref="form" label-position="top"  label-width="130px" size="mini"> 
                 ...
                 <el-form-item label="Loại giấy">
                  <el-select v-model="form.Type" @change="changeType" placeholder="Chọn mã" >
                    <el-option
  ...
  :rows="2" v-model="form.Reason" ></el-input>
    ...
    <div slot="footer" style="margin:10px;margin-left:auto;margin-right:20px">
      ...
      <el-button type="success" icon="el-icon-success" @click="creatrMissionNP" >Thêm</el-button>
    </div>
  </modal>
</template>
<script>
...
export default {
  ...
  data() {  
    return {
      ...
      listDate:[],
      ReasonLst:[{value: 'Bản thân kết hôn (03 ngày)'},{
  ...
  methods: {
    ...
    changeReason(){
      this.form.TimeStart  = Date.parse(this.form.TimeStart.toString('yyyy-MM-dd'))
        ...
        if(this.form.Reason == 'Bố mẹ 2 bên vợ chồng chết; vợ, chồng, con chết (03 ngày )') {
          var a = new Date(this.form.TimeStart)
          a.setDate(this.form.TimeStart.getDate()+3)
          this.form.TimeEnd = a
        }
    ...
    creatrMissionNP(){
      ...
      //  this.form.TimeEnd = Date.parse(this.form.TimeEnd).toString('yyyy-MM-dd')
