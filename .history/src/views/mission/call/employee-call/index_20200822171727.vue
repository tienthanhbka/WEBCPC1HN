<template>
  <div class="employee-mission-container">
    <div>
      ...
      <el-button type="success" icon="el-icon-document" :loading="downloading"
        @click="exportCall" class="inline-block">Xuất Báo cáo</el-button>
      <router-link to="/mission-call/call/my-em/report">
        <el-button type="success" icon="el-icon-document" class="inline-block">Thống kê nhóm</el-button>
      </router-link>
    </div>
    ...
    <div>
      ...
      >
        <el-table-column type="expand">
          <template slot-scope="scope">
            <call-info :imageLoaded="scope.row.imageLoaded" :call="scope.row"></call-info>
          </template>
        </el-table-column>
        <!-- <el-table-column
      type="selection" 
      ...
      :filters="[{text: 'Đã đánh giá', value: '10'},{text: 'Chưa đánh giá', value: '0'}]">
        ...
        >
          <template slot="header">
            <el-tooltip content="Gửi đánh giá" placement="top">
              ...
              <el-button @click="uploadEvaluate()" size="mini">
                <i class="el-icon-loading" v-show="false" id="is-upload-evaluate"></i>
          ...
          <template slot-scope="scope">
            <el-checkbox :disabled="scope.row.status<6" v-model="scope.row.evaluate"></el-checkbox>
          </template>
        </el-table-column>
        <el-table-column
          label="Trạng thái"
        ...
        >
          <template slot-scope="scope">
            <el-tag
              :type="scope.row.status | callStatusColor"
            >{{ scope.row.status | callStatusText }}</el-tag>
            <el-tag v-if="scope.row.numImg>0" type="info">{{scope.row.numImg}} ảnh</el-tag>
          </template>
        </el-table-column>
        <el-table-column
          prop="nameEmployee"
        ...
        <el-table-column label="Checkin" width="150" align="center">
          <template slot-scope="scope">
            <el-tooltip
              :content="scope.row.timeCheckIn.substring(0,16)+' -- ' + scope.row.nameLocationCheckIn"
              placement="top"
            >
              <div>
                {{scope.row.timeCheckIn.substring(0,16)}}
                <el-button
                  @click="openLocation(scope.row.latitudeCheckIn,scope.row.longitudeCheckIn)"
                  type="text"
                  style="color:#8e44ad"
                >
        ...
        <el-table-column label="Checkout" width="150" align="center">
          <template slot-scope="scope">
            <el-tooltip
              :content="scope.row.timeCheckOut.substring(0,16)+' -- ' + scope.row.nameLocationCheckOut"
              placement="top"
            >
              <div>
                {{scope.row.timeCheckOut.substring(0,16)}}
                <el-button
                  @click="openLocation(scope.row.latitudeCheckOut,scope.row.longitudeCheckOut)"
                  type="text"
                  style="color:#8e44ad"
                >
                  <i class="fas fa-map-marker-alt"></i>
        ...
        <el-table-column prop="namePlace" label="Khách hàng tổ chức" width="300"></el-table-column>
        <el-table-column prop="nameCustomer" width="170" label="Khách hàng cá nhân"></el-table-column>
        <el-table-column prop="plan" label="Nội dung" width="300"></el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    exportCall(){
      ...
      const tHeader = ['Mã Call','ID Group','ID Nhân viên','Tên Nhân viên','Khách hàng tổ chức','Nhiệm vụ','Khách hàng cá nhân','Chức vụ','Kế hoạch','Kết quả',
      'Trạng thái','Thời gian CheckIn','Thời  gian CheckOut','Địa điểm CheckIn','Địa điểm CheckOut','Thời gian kế hoạch','Thời gian duyệt','Thời gian hoàn thành'];
      const filterVal = ['idCall','idGroup','idEmployee','nameEmployee','namePlace','nameMission','nameCustomer','positionCustomer','plan','note','status','timeCheckIn','timeCheckOut',
      'nameLocationCheckIn','nameLocationCheckOut','timePlan','checktime','completetime'];
      var self = this;
      GetCallByIdGroup({
        ...
        excel.export_json_to_excel({
          ...
          filename: "Báo cáo từ "+Date.parse(self.DateBegin).toString('dd-MM-yyyy')+' đến '+Date.parse(self.DateEnd).toString('dd-MM-yyyy'), //Optional
    ...
    formatJson(filterVal, jsonData) {
      return jsonData.map(v => filterVal.map(j => {
        if (j === 'checktime' ||j === 'completetime' ||j === 'timeCheckIn' ||j === 'timeCheckOut' ||j === 'timePlan') {
    ...
    checkAll(e) {
      ...
      if (e) {
        this.tableData.forEach(element => {
          if (element.status >= 6&&!element.namePlace.includes('CPC1')) element.evaluate = true;
        });
      } else {
        this.tableData.forEach(element => {
