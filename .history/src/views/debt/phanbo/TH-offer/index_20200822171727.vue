<template>
  <div>
    ...
    <el-dialog title="Dự án" :visible.sync="prjectModalVisible" custom-class="modal-class">
    ...
    <el-dialog title="Sản phẩm" :visible.sync="itemModalVisible" custom-class="modal-class">
      <item-modal-content
        :coupon="curCoupon"
        :project="curProject"
    ...
    <el-input style="width:200px" v-model="search" type="text" placeholder="Mã KH | Tên KH | Tên NV"></el-input>
      ...
      <!-- <el-button :v-show="false" style="margin-left:0px" @click="ExportExcel" type="success" class="inline-block">
        <i class="fas fa-file-download"></i> Xuất Excel
        <i v-if="downloadingFile" class="el-icon-loading"></i>
      </el-button>
      <el-button :v-show="false" v-if="!isGroupPage"
        style="margin-left:0px"
      ...
      <el-tooltip content="Xem hướng dân">
       <a href="https://docs.google.com/presentation/d/1WfbWdX6OjaZUZsLlm1gkBPvt80z_nkIehx9io9LDW8s/edit#slide=id.g617858ad81_0_10" target="_blank">
<el-button size="small" type="primary"  circle=""><i class="fas fa-question"></i></el-button>
</a>
      ...
      >
        <el-table-column type="expand">
          <template slot-scope="scope">
            ...
            ></th-projects-c>
            <item :type="type" :isGroupPage="isGroupPage" :phanboCoupon="scope.row" @addItemClicked="onOpenItemModal" v-else></item>
          </template>
        </el-table-column>
          ...
          <template slot-scope="scope">
            ...
            <el-popover placement="top-start" trigger="hover" v-if="scope.row.status!=10">
              <div>
                ...
                <el-button
                  v-if="scope.row.status!=10&&(scope.row.status==1||(scope.row.status<7 && scope.row.lv==2)||(scope.row.status<9 && scope.row.lv==3))"
                  style="color:#c0392b"
                  type="text"
                  size="medium"
                  icon="el-icon-delete"
