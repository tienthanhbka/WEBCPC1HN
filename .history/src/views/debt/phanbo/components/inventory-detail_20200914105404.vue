<template>
  <div>
    ...
    >
      <customer-cn-modal-content :cusNo="info.cusNo" @cusChoosed="onCusChoosed"></customer-cn-modal-content>
    </el-dialog>
    <el-button size="small" type="primary" @click="fetchData">
      <i class="fas fa-sync-alt"></i>
    </el-button>
    <!-- <el-button size="small" type="success" @click="saveTemp">
      Lưu lại bản nháp
    </el-button>-->
    <div v-loading="isLoading" style="min-height:30px">
      <div v-for="(item, index) in tableData" :key="index">
        <div :class="validRow(item)?'in-row':'in-row error'">
          <span v-if="!validRow(item)">(Không có dữ liệu đầu kì, cuối kì!!!)</span>
          <el-button
            size="small"
            type="primary"
          ...
          <span v-if="!validQuantity(item)" style="color:red">Hãy chia đủ số lượng</span>
          <el-table
          ...
          >
            ...
            <el-table-column
              :label="'SL Tiêu thụ (max ='+numberFormat(item.SellQuantity)+')'"
              min-width="150"
            >
              ...
              <template slot-scope="{row}">
                ...
                ></el-input> -->
                 <!-- <el-checkbox-group controls-position="right"  v-if="!disableEdit"  v-model="row.Pay">
                  ...
                  <el-radio controls-position="right"  v-if="!disableEdit" v-model="row.Pay" label="1">CK</el-radio>
