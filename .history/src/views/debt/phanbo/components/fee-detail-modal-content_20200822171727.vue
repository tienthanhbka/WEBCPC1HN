  ...
  <div>
    <el-tabs type="border-card">
      <el-tab-pane label="Tốc độ đẩy hàng">
        <inventory-detail :visible="visible" :disableEdit="disableEdit" :info="info"></inventory-detail>
      </el-tab-pane>
    ...
    <div style="text-align:right;margin-top:10px">
      <el-button type="success" @click="sendRequest" v-if="!disableEdit">
        <template v-if="info.rowID">Sửa đề xuất</template>
        <template v-else>Gửi đề xuất</template>
      </el-button>
    </div>
  </div>
</template>
        ...
        if (this.info.inventoryData) {
          ...
          if (!valid) {
            ...
            if (this.info.rowID) {
              ...
              if (this.info.inventoryData != null&&this.info.inventoryData.length>0 ) {
                CreateFeeRequest({
                  data: this.info
                }).then(res => {
                ...
                this.$message({
                  message: "Hãy bấm cập nhật tốc độ đẩy hàng, hoặc bỏ tính mô B2 nếu không có tồn kho",
                  type: "error"
                });
              }
            }
