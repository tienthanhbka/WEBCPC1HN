<template>
  <div>
    ...
    >
      <el-button
        type="danger"
        v-if="!disabled && form.status != null"
        @click="cancelFee()"
        >Hủy phiếu</el-button
      >
...
import { getThreeMonth } from "@/api/index";
export default {
  components: {
    CusModalContent,
    ItemModalContent,
  ...
  data() {
    return {
      ...
      OptionsTimeStart: {
        disabledDate(time) {
          let admin =
            "0987299101;0939655142;0978035800;0983420302;0394622492;0985141095;0396975705";
          return admin.includes(Cookies.get("idEmployee"))
            ? false
            : time.getTime() < getThreeMonth();
        }
      }
    };
  },
  methods: {
    ...
    openFeeTable() {
      if (this.form.typeCus.length == 0) {
        ...
        this.form.end = Date.parse(this.form.end).toString(
          "yyyy-MM-dd HH:mm:ss"
        );
        this.feeDetailModalVisible = true;
      }
    },
    changeDateStart(time) {
      // const now = new Date();
      // if (now.getMonth() - time.getMonth() > 3) {
      //   this.$notify({
      //     title: "Xảy ra lỗi",
      //     message: "Thời gian được chọn vượt quá hạn cho phép",
      //     type: "error"
      //   });
      //   this.form.start = "";
      //   return;
      // }
      //console.log(time);
      this.form.start = new Date(time.getFullYear(), time.getMonth(), 1);
      //console.log(this.form.start);
    },
    changeDateEnd(time) {
      this.form.end = new Date(time.getFullYear(), time.getMonth() + 1, 0);
