<template>
  <div>
    <el-dialog title="Đề nghị chi phí" :visible.sync="feeModalVisible" custom-class="modal-class">
      <fee-request-modal @feeRequestAdded="onFeeRequestAdded" @feeRequestEdited="onFeeRequestEdited" :coupon="this.currentCoupon"></fee-request-modal>
    </el-dialog>
    <div v-if="!isGroupPage">
      <el-button type="primary" icon="el-icon-refresh" class="inline-block" @click="fetchData"></el-button>
      <el-button @click="openModalFee()" type="warning"><i class="fas fa-plus"></i> Phiếu đề nghị chi phí</el-button>
      <div style="display: inline-block;">
        <el-input placeholder="Tên NV | Mã KH" v-model="search" class="input-with-select">
    ...
    >
      <el-table-column width="70">
        <template slot-scope="scope">
          <el-button type="text" style="padding:0px" @click="openModalFee(scope.row)">Chi tiết</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column  width="200" label="Thời giam tính">
          <template slot-scope="{row}">
              <span style="color:#6c5ce7"><i class="fas fa-calendar"></i> {{Date.parse(row.StartTime).toString('dd/MM/yyyy')}} - {{Date.parse(row.EndTime).toString('dd/MM/yyyy')}} </span>
          </template>
      </el-table-column>
      ...
      <el-table-column  width="120" label="Trạng thái">
          <template slot-scope="{row}">
             <el-tag slot="reference" :type="row.Status|requestHeaderStatusColor ">
...
export default {
  ...
  methods: {
    onFeeRequestAdded(fee){
      ...
      console.log(this.feeModalVisible);
