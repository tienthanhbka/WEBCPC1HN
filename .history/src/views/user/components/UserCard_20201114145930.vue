<template>
  <div>
    <div class="user-profile">
      <div v-if="displayUpdate" class=" text-center bt-update">    <el-button @click="updateInfor" size="mini" type="primary" plain><i class="far fa-edit"></i></el-button> </div>
      <div v-if="!displayUpdate" class=" text-center bt-update">    <el-button @click="acceptInfor" size="mini" type="primary" plain><i class="fas fa-check-double"></i></el-button> </div>
      ...
      <div class="user-name text-center"><span >{{ user[0].EmployeeName }}</span> <span v-if="user[0].Status==2"> <i style="color:green;" class="far fa-check-circle"></i></span> <span v-if="user[0].Status==0">(Cập nhật lại)</span></div>
        <div class="user-name text-center"><span >{{ user[0].EmployeeCode }}</span> </div>
         <div v-if="user[0].Status<2" class="user-name text-center"><span >{{ user[0].Note }}</span> </div>
         <div><router-link
                  :to="'/lo-trinh-cong-danh/'+ user[0].EmployeeID">
      ...
      <div class="box-social">
        <el-table :data="infoT()" :show-header="false">
          <el-table-column   label="Name">
            <template v-if="scope.row.value !=null" slot-scope="scope">{{ scope.row.name }}</template>
          </el-table-column>
          <el-table-column  label="Count"  align="right" >
            <template v-if="scope.row.value !=null" slot-scope="scope">{{ scope.row.value }}</template>
          </el-table-column>
        </el-table>
      </div>
      ...
      <span slot="footer" class="dialog-footer">
       ...
       <el-dialog title="Phê duyệt thông tin" :visible.sync="dialogFormAccept">
        <acceptInfor :user="user" @click-something="acceptOK" @cancel="cancelInfo"></acceptInfor>
        
      <span slot="footer" class="dialog-footer">
      </span>
    </el-dialog>

      <el-dialog title="Yêu cầu cập nhật lại thông tin" :visible.sync="dialogFormNoAccept">
        <cancel :row="user" @noacceptOK="noacceptOK" @cancel="cancelInfo"></cancel>
        
      <span slot="footer" class="dialog-footer">
      </span>
    </el-dialog>

     <el-dialog title="Cập nhật thông tin gia đình" :visible.sync="dialogFormUpdateRe">
      ...
      <span slot="footer" class="dialog-footer">
       ...
       <el-dialog class="el-img" :visible.sync="imgVisible">
        <img :src="'https://icpc1hn.work/api/'+'File/GetImageEmployee/'+user[0].Img+'?token='+token+'&EmployeeID='+user[0].EmployeeID">
