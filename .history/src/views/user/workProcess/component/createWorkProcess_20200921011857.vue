<template>
    ...
    >
      ...
      <el-form-item label="Quá trình hợp đồng" prop="Type">
        <el-select v-model="form.Type" placeholder="Chọn" @change="changeType"  style="width:100%">
          <el-option
            v-for="item in Type"
            :key="item.value"
            :label="item.value"
...
<el-row>
 ...
 <el-form-item label="Mức lương đóng BHXH">
       ...
       <el-select v-model="form.BHXH" filterable allow-create default-first-option placeholder="Chọn"  style="width:100%">
          <el-option
            v-for="item in Money"
            :key="item.value"
            :label="item.value"
...
export default {
    ...
    methods:{
      changeType(){
        this.form.DateStart  = Date.parse(this.form.DateStart.toString('yyyy-MM-dd'))
        ...
        modalOK(){
           ...
           this.form.DateStart = Date.parse(this.form.DateStart.toString('yyyy-MM-dd'))
            CreateEmployeeProcessWork(this.form).then(res=>{
                this.$notify({
                    title:'Thành công',
