<template>
  <div  shadow="always" style="margin: auto; position: relative; padding-bottom: 100px;">
    <div>
        <el-card >
            <div style="font-weight: bold; font-size: 30px; color: #777777;">
                 Hướng dẫn: {{ lines.Content }}
            </div>
          
           
            <div style=" font-size: 15px; margin-top: 10px; color: #777777;">
                <span style="font-weight: bold;">Hiệu lực:</span> {{ lines.TimeEnd }}
            </div>

             <div style=" font-size: 15px; margin-top: 10px; color: #777777;">
                <span style="font-weight: bold;">Đối tượng áp dụng:</span> {{ lines.Office }}
            </div>
    ...
    <div v-if="lines.Link != ''" v-html="lines.Link">
      
    </div>

     <div  v-for="(line, index) in videos" :key="index" class="row">
      <div>
          <el-card>
                ...
                <div style="width: 100%; height: 100%;"  class="grid-content bg-purple-light">
                    ...
                    <vue-core-video-player :src="'https://icpc1hn.work/api/'+'File/GetFileReguVideo2/'+line+'?token='+token"></vue-core-video-player>
                    <!-- <div>  {{ line }}</div> -->
                </div>
