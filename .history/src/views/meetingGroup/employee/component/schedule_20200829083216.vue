      ...
      handleIconClick2(ev) {
       ...
       createFilter2(queryString) {
        return (link) => {
          ...
          //console.log(link)
          return (link.ProductName.toLowerCase().includes(queryString.toLowerCase()) === true);
        };
      },
   modalOK(){
      ...
      if(OK = true){
       ...
       GetMeetingGroup(req).then(response => {
        ...
        //    this.total = (response.PageNumber)*(response.RowspPage)
          if(response.Data.length < 4 || this.Group == "CNHN-CONGNO"){
        ...
        this.$message({
          message: "Bạn đã đăng kí 4 lần trong tháng "+ Date.parse(this.form.month).toString('MM'),
          type: "warning"
        })
      }
          }
