<template>
  <div>
    ...
    >
      ...
      <el-form-item label="Nguyên nhân" v-if="Offer.OfferType == 2">
        <el-input
          class="selectIDGroup"
      ...
      <el-form-item label="Mô tả chi tiết cải tiến" v-if="Offer.OfferType == 2">
        <el-input
          class="selectIDGroup"
      ...
      <el-form-item
        label="File đề xuất"
        v-if="Offer.UpFile && Offer.OfferType == 2"
      >
      ...
      <el-form-item v-if="Offer.Comment">
        <span slot="label" style="color:#02bc77; font-size:10pt;"
          >Phụ trách thực hiện</span
        >
        <div v-for="(item, index) in Offer.OfferIdeaLineLst" :key="index">
          <el-input
            class="selectIDGroup"
            v-model="Offer.EmployeeName"
            style="width:100%;"
          >
          </el-input>
        </div>
...
import { GetOfferIdeaByID } from "@/api/offerIT";
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      GetOfferIdeaByID(req).then(res => {
        if (res.RespCode == 0) {
          this.Offer.OfferIdeaLineLst = res.OfferIdeaInfo.OfferIdeaLineLst;
        } else {
          this.$message({
