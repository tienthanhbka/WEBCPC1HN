...
<div class="orderhcm-container">
    <div class="orderhcm-header">
        ...
        <div class="inline-block">
         ...
         <div class="inline-block">
            <el-button type="primary" @click="createHand">Tạo phiếu bàn giao</el-button>
        </div>
        <!-- <div class="inline-block">
    ...
    <el-dialog
        ...
        >
         <createHandOver  :groupList="groupList"  @createOK="createOK" ></createHandOver>
    </el-dialog>
     ...
     <el-dialog
       ...
       >
         <acceptHandOver :row="row" :groupList="groupList"  @acceptOK="acceptOK" ></acceptHandOver>
    </el-dialog>
      ...
      <el-dialog
       ...
       >
         <delHandOver :row="row" :groupList="groupList"  @delOK="delOK" ></delHandOver>
    </el-dialog>
    ...
    <el-dialog
        ...
        width="700.44pt"
        >
         <viewHandOver :row="rowSelect" :groupList="groupList" @sendAcceptOK="sendAcceptOK"  @acceptOK="acceptOK" ></viewHandOver>
    </el-dialog>
    ...
    >
      <el-table-column  align="center" prop="No_" label="STT" width="95">
        <template slot-scope="scope">{{ (scope.$index+1) }}
          <el-tooltip content="Duyệt" placement="top"><el-button @click="AcceptHandOver(scope.row)" type="text" > <i class="fas fa-check-double"></i></el-button></el-tooltip>
      ...
      <el-table-column  label="Loại" width="150">
        ...
        <el-table-column  class-name="status-col" label="Danh mục"  align="center">
        <template slot-scope="scope">{{ scope.row.HandOver  }}</template>
      </el-table-column>
      <el-table-column  class-name="status-col" label="Số lượng"  align="center">
        <template slot-scope="scope">{{ scope.row.Number }}</template>
      </el-table-column>
       <el-table-column  class-name="status-col" label="Đơn vị"  align="center">
        <template slot-scope="scope">{{ scope.row.Unit }}</template>
      </el-table-column>
       <el-table-column  class-name="status-col" label="Hiện trạng"  align="center">
        <template slot-scope="scope">{{ scope.row.CurrentStatus }}</template>
      </el-table-column>
       <el-table-column  class-name="status-col" label="Người tiếp nhận"  align="center">
        <template slot-scope="scope">{{ scope.row.EmployeeNameNext }}</template>
      </el-table-column>
      ...
      <el-table-column  class-name="status-col" label="Trạng thái" width="150" align="center">
        <template slot-scope="scope">
          ...
          >{{ scope.row.Status|orderStatusColor }}</el-tag>
        </template>
      </el-table-column>
        ...
        <el-table-column align="center" width="175px" label="">
           <template slot-scope="scope">
           <el-tooltip content="Xem chi tiết" placement="top"><el-button @click="ViewHandOver(scope.row)" type="text" > <i class="far fa-eye"></i></el-button></el-tooltip>
        <el-tooltip content="Duyệt" placement="top"><el-button @click="AcceptHandOver(scope.row)" type="text" > <i class="fas fa-check-double"></i></el-button></el-tooltip>
         <el-tooltip content="Xóa" placement="top"><el-button @click="DelHandOver(scope.row)" type="text" > <i style="color: red;" class="fas fa-trash-alt"></i></el-button></el-tooltip>
         ...
         :filters="[{text: 'Hoàn thành', value: '10'},{text: 'Đã xong', value: '6'}, {text: 'Đang xử lý', value: '3'},{text: 'Đã dừng', value: '2'}, {text: 'Mới tạo', value: '1'},{text: 'Đã hủy', value: '0'}]"  >
             ...
             <el-table-column label="Ngày bàn giao xong" align="center" min-width="100">
                <template slot-scope="scope">
...
<script>
  ...
  filters:{
   ...
   components:{
     CreateHandOver,AcceptHandOver,ViewHandOver,SendAccept,DelHandOver
   },
  props: {
          ...
          delOK(){
           ...
           sendAcceptOK(){
            ...
            expandRow(row, expandedRows){
                 if (row.lines == null) {     
                    var id = row.RowID;
                    GetHandOverLine({RowID : id ,token: Cookies.get('token')}).then(res => {
                    row.lines = res.Data;
                    this.obj= res.Data;
                    this.error = res.Data;
